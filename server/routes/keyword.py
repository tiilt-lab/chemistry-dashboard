...
def add_keyword_list(user, **kwargs):
    ...
    valid, message = KeywordList.verify_fields(name=new_name, keywords=new_keywords)
    if not valid:
    ...
    if new_name != None or new_keywords != None:
        ...
        new_keyword_list = database.update_keyword_list(new_keyword_list.id, new_name, new_keywords)
...
def update_keyword_list(keyword_list_id, **kwargs):
    ...
    if not valid:
        return json_response({'message': message}, 400)
    if new_name != None or new_keywords != None:
        keyword_list = database.update_keyword_list(keyword_list_id, new_name, new_keywords)
        return json_response(keyword_list.json())
    return json_response({'message': 'Keyword list not found'}, 400)
