...
# Speaker cache
_speakers_cache: Optional[Set[str]] = None
_speakers_cache_time: Optional[datetime] = None

# Session name cache
_session_names_cache: Optional[List[tuple]] = None
_session_names_cache_time: Optional[datetime] = None
...
def _load_speakers_from_db() -> Set[str]:
    ...
    """
    global _speakers_cache, _speakers_cache_time

    # Check cache
    if _speakers_cache is not None and _speakers_cache_time is not None:
        age = (datetime.now() - _speakers_cache_time).total_seconds()
    ...
    try:
        ...
        _speakers_cache_time = datetime.now()
        logger.info(f"[Memory] Loaded {len(speakers)} speakers from database")
        return speakers
...
def _load_session_names_from_db() -> List[tuple]:
    ...
    """
    global _session_names_cache, _session_names_cache_time

    # Check cache
    if _session_names_cache is not None and _session_names_cache_time is not None:
        age = (datetime.now() - _session_names_cache_time).total_seconds()
        if age < CACHE_TTL_SECONDS:
    ...
    try:
        ...
        for session_id, session_name, device_name in rows:
            ...
            # Device name (e.g., "midnight", "kha4", "makerspace")
            if device_name:
                name_to_id[device_name.lower().strip()] = session_id
        ...
        _session_names_cache = result
        _session_names_cache_time = datetime.now()
        logger.info(f"[Memory] Loaded {len(result)} session name patterns from database ({len(name_to_id)} full names, {len(word_to_id)} words)")
        return result
...
def clear_caches():
    ...
    _speakers_cache_time = None
    _session_names_cache = None
    _session_names_cache_time = None
...
def get_memory(conversation_id: str) -> ConversationMemory:
    ...
    if conversation_id not in _memory_store:
        _memory_store[conversation_id] = ConversationMemory(conversation_id)
        logger.info(f"[Memory] Created new memory for conversation {conversation_id}")
...
def clear_memory(conversation_id: str):
    ...
    if conversation_id in _memory_store:
        ...
        logger.info(f"[Memory] Cleared memory for conversation {conversation_id}")
