"""
Simplified Graph for BLINC Agent Baseline (Transcript-Only)

A minimal LangGraph-based architecture using transcript-only tools:
1. process_input - Extract context, resolve references
2. agent_loop - ReAct loop with tool calling (4 tools only)
3. format_response - Final formatting

Same architecture as V7 but with restricted tool access.
"""
...
class AgentState(TypedDict, total=False):
    """
    Minimal state for the baseline agent.
...
def process_input(state: AgentState) -> AgentState:
    ...
    """
    logger.info(f"[Baseline Graph] Processing input: {state.get('original_query', '')[:50]}...")
    ...
    return {
        **state,
        'current_query': query,
        'session_focus': session_id or memory.session_focus,
...
def run_react_agent(state: AgentState) -> AgentState:
    ...
    logger.info("[Baseline Graph] Running ReAct agent")
    ...
    try:
        # Run the agent
        response: AgentResponse = run_agent(conversation_id, query)
        ...
        import traceback
        logger.error(f"[Baseline Graph] Agent error: {e}")
        logger.error(f"[Baseline Graph] Traceback:\n{traceback.format_exc()}")
        elapsed_ms = (time.time() - start_time) * 1000
...
def format_response(state: AgentState) -> AgentState:
    ...
    """
    logger.info("[Baseline Graph] Formatting response")
...
def reset_conversation(conversation_id: str):
    ...
    logger.info(f"[Baseline Graph] Reset conversation: {conversation_id}")
