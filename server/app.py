...
# Redis
r = redis.Redis(host=cf.redis(), port=6379, db=0)
...
if cf.cloud():
	socketio = SocketIO(app, log=logger, cors_allowed_origins=[cf.domain(),"http://server:5000","http://audio_processor", "http://nginx", "http://server", "http://127.0.0.1", "http://localhost"], manage_session=False, message_queue="redis://"+cf.redis())
else:
	socketio = SocketIO(app, log=logger, cors_allowed_origins=[cf.domain(),"http://server:5000","http://audio_processor", "http://nginx", "http://server", "http://127.0.0.1", "http://localhost"],manage_session=False, message_queue="redis://"+cf.redis())
