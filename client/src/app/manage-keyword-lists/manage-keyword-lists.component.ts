...
export class ManageKeywordListsComponent implements OnInit {
  constructor(private router: Router, private keywordService: KeywordService) {}
  ...
  createTopicModel() {
    this.keywordService.getTopics().subscribe((topics) => {
      this.topics = topics;
  ...
  confirmDeleteKeywordList() {
    this.keywordService
      .deleteKeywordList(this.keywordListToDelete.id)
      .subscribe(
        (result) => {
          this.keywordLists = this.keywordLists.filter(
            (kl) => kl.id !== this.keywordListToDelete.id
          );
