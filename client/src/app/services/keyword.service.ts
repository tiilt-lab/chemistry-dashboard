...
export class KeywordService {
  ...
  getKeywordList(keywordListID): Observable<any> {
    return this.api.get('api/v1/keyword_lists/' + keywordListID).map(response => {
  ...
  updateKeywordList(keywordListID: number, name: string, keywords: any[]): Observable<any> {
    const body = {
      'name': name,
      'keywords': keywords
    };
    return this.api.put('api/v1/keyword_lists/' + keywordListID, body).map(response => {
      return KeywordListModel.fromJson(response.json());
    });
  }

  deleteKeywordList(keywordListID: number): Observable<any> {
    return this.api.delete('api/v1/keyword_lists/' + keywordListID).map(response => {
      return response.json();
    });
  }

  getKeywordListItems(keywordListID: number): Observable<any> {
    return this.api.get('api/v1/keyword_lists/' + keywordListID + '/keywords').map(response => {
