...
export class AuthService {
  ...
  lockUser(userId): Observable<any> {
    return this.api.post('api/v1/admin/users/' + userId + '/lock', {}).map(response => {
      return UserModel.fromJsonList(response.json());
    });
  }

  unlockUser(userId): Observable<any> {
    return this.api.post('api/v1/admin/users/' + userId + '/unlock', {}).map(response => {
      return UserModel.fromJsonList(response.json());
    });
  }
  ...
  changeUserRole(userId, role: string): Observable<any> {
    ...
    return this.api.post('api/v1/admin/users/' + userId + '/role', body).map(response => {
      return UserModel.fromJsonList(response.json());
    });
  }

  resetUserPassword(userId): Observable<any> {
    return this.api.post('api/v1/admin/users/' + userId + '/reset', {}).map(response => {
      return response.json();
    });
  }
  ...
  getDeviceLogs(deviceId: number) {
    return this.api.get('api/v1/admin/devices/' + deviceId + '/logs').map(response => {
      const jsonData = response.json();
      const dataUrl = jsonData['data'];
      fetch(dataUrl).then(res => res.blob()).then(blob => {
        const blobUrl = URL.createObjectURL(blob);
  ...
  deleteServerLogs(type: string) {
    ...
    return this.api.delete('api/v1/admin/server/logs', query).map(response => {
      return response.json();
    });
  }

  deleteDeviceLogs(deviceId: number) {
    return this.api.delete('api/v1/admin/devices/' + deviceId + '/logs').map(response => {
      return response.json();
    });
  }

  allowAPIAccess(userId: number) {
    return this.api.post('api/v1/admin/users/' + userId + '/api', {}).map(response => {
      return response.json();
    });
  }

  revokeAPIAccess(userId: number) {
    return this.api.delete('api/v1/admin/users/' + userId + '/api').map(response => {
      return response.json();
    });
  }
