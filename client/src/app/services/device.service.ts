...
export class DeviceService {
  ...
  getDevices(archived: boolean = false, connected: boolean = null, inUse: boolean = null, isPod: boolean = true): Observable<any> {
    const query = {
      'archived': archived,
      'connected': connected,
      'inUse': inUse,
  ...
  blinkPod(deviceId, op): Observable<any> {
    ...
    return this.api.post(`api/v1/devices/${deviceId}/blink`, body).map(response => {
      return response.json();
    });
  }
