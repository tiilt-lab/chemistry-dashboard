      ...
      <div className={style["list-container"]} style = {{"max-width": adjDim(375) + 'px',}}>
        ...
        {(!props.isLoading && props.sessions.length === 0 && props.folders.length === 0)
          ...
          <div className={style["empty-session-list"]}>
            <div className={style["load-text"]}> No Discussions or Folders </div>
            <div className={style["load-text-description"]}> Tap the buttons below to record your first discussion or create your first folder. </div>
          </div>
          :
          <></>}
        ...
        <div className={style["breadcrumb-container"]}>
          {props.breadcrumbs.length > 0 ?
            <div onClick={props.goBackToPrevious}>
              <img alt="breadcumbs" src={breadcrumbIcon} className={style["breadcrumb-arrow-icon"]} />
            </div>
            : <></>}
          ...
          <div>
            <ul className={style.list}>
              {props.displayedFolders.map((folder,index) => (
                <li key={index} className={style["folder-item"]}>
                  <img alt="folder" src={folderIcon} className={style["folder-icon"]} />
                  <div className={style["folder-title"]} onClick={() => { props.displayFolder(folder.id) }}>{folder.name}</div>
                  <div className={style["folder-options"]}>
                    <AppContextMenu>
                      <div className={style2["menu-item"]} onClick={() => { props.openFolderDialog("RenameFolder", folder) }}>Edit Name</div>
                      <div className={style2["menu-item"]} onClick={() => { props.openFolderDialog("MoveFolder", folder) }}> Move To...</div>
                      <div className={`${style2["menu-item"]} ${style2["red"]}` } onClick={() => { props.openFolderDialog("DeleteFolder", folder) }} > Delete</div >
                    </AppContextMenu >  
                  </div >
                </li >
              ))}
                ...
                {props.displayedSessions.map((session,index) => (
                  <li key={index} className={style["session-item"]}>
                    ...
                    <div className={style["session-options"]}>
                      <AppContextMenu>
                        <div className={style2["menu-item"]} onClick={() => { props.openSessionDialog("RenameSession", session) }}>Edit Name</div>
                        <div className={style2["menu-item"]} onClick={() => { props.openSessionDialog("MoveSession", session) }} > Move To...</div >
                        {!session.recording ? <div className={`${style2["menu-item"]} ${style2["red"]}` } onClick={() => { props.openSessionDialog("DeleteSession", session) }} > Delete</div > : <></>}
                        {session.recording ? <div className={`${style2["menu-item"]} ${style2["red"]}` } onClick={() => { props.endSession(session) }} > End</div > : <></>}
                      </AppContextMenu >
                    </div >
                    <div className={style["session-title"]}>{session.name}</div>
                  </li >
                ))}
              </ul >
            </div >
...

        {
          (props.folders.length > 0 || props.sessions.length > 0) ?
            <div>
              {(props.displayedFolders.length === 0 && props.displayedSessions.length === 0) ?
                <div >
                  <div className={style["folder-empty"]}> Folder Empty </div>
                </div>
                :
                <></>
              }
...
<></>
 ...
 {(props.currentForm === "DeleteSession")?
  ...
  :
  <div className={style["basic-button"]} onClick={() => props.addFolder(document.getElementById('txtName').value,props.breadcrumbs[props.breadcrumbs.length - 1].id)} > Confirm</div >}
 ...
 {props.folderSelect !== null ? <button className={style["basic-button"]} onClick={() => props.moveFolder(props.folderSelect)} > OK</button >:<></>}
 <button className={style["cancel-button"]} onClick={props.closeDialog}> Cancel</button >
  </div >
  :
  <></>
  ...
  <div className={style["dialog-window"]} style = {{"min-width": adjDim(270) + 'px',}} >
    ...
    <AppFolderSelectComponent
     ...
     />
    {/* <app-folder-select #folderSelect [folders] = "selectableFolders" ></app - folder - select > */}
  {props.folderSelect!== null ? <button className={style["basic-button"]} onClick={() => props.moveSession(props.folderSelect)} > OK</button >:<></>}
  <button className={style["cancel-button"]} onClick={props.closeDialog}> Cancel</button >
  </div >
  :
  <></>}
