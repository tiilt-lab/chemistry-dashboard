    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4">
            <div className="mb-3 flex items-center justify-between">
                ...
                <div className="text-sm text-tiilt-muted">Total: {props.formatSeconds(props.duration)}</div>
            </div>
            ...
            <div className="relative w-full select-none">
                ...
                <div className="relative h-10">
                        ...
                        {props.ticks.map((t, idx) => {
                            ...
                            return (
                                <div key={`tick-${t}-${idx}`} className="absolute top-0 h-full" style={{ left: `${left}%` }}>
                                    <div className="w-px h-full bg-tiilt-line/60" />
                                    <div className="absolute -translate-x-1/2 mt-2 text-[10px] text-tiilt-muted">{Math.round(t)}s</div>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    {props.hover && (
                        <div
                            className="absolute -top-12 left-0 transform -translate-x-1/2 rounded-xl shadow-lg bg-white px-3 py-2 text-xs border border-tiilt-line"
                        ...
                        >
                            ...
                            <div className="text-tiilt-muted">
                                {props.formatSeconds(props.hover.start)} → {props.formatSeconds(props.hover.end)}
                            </div>
                            <div className="text-tiilt-muted">{props.hover.duration.toFixed(2)}s</div>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-wrap items-center gap-3">
                    {(Object.keys(props.CATEGORY_COLORS)).map((e) => (
                        <div key={e} className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-tiilt-ink">{props.CATEGORY_LABELS[e]}</span>
                        </div>
                    ))}
                </div>
                ...
                {/* Distribution bar */}
                <div className="relative w-full h-3 rounded-full overflow-hidden bg-tiilt-line/40">
                    {(Object.keys(props.CATEGORY_COLORS)).map((e) => {
