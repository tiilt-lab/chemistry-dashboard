...
function Appheader(props){
    ...
    <div className={style["header-grid"]}>
       {props.leftText!== false
        ...
        <div className={style.center}>{props.editMode ? <input class={style.center} type="text" defaultValue={props.title} style={{visibility: 'visible', width: '200px', 'border-width': '0px', overflow: 'scroll', 'line-height': '50px',}} onKeyUp={(event) => props.changeInputVal(event.target.value)} /> : props.title}</div>
