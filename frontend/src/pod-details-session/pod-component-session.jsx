...
import { PodComponentSeesionPages } from "./html-pages";

function PodComponentSession() {
  ...
  useEffect(() => {
    if (speakers && speakers.length > 0){
      setSelectedSpkrId1(speakers[0].id)
      setSelectedSpkralias(speakers[0].alias)
  ...
  const loading = () => {
   ...
   const getSessionSpeakers = () => {
      const fetchData = new SessionService().getSessionSpeakers(session.id);
      fetchData.then(
        (response) => {
            ...
            response.json().then((jsonObj) => {
              ...
              if(input && input.length){
                input.map((spkr, index)=>{
                  if(identified_speakers.indexOf(spkr.id) == -1){
                    uniqueSpeakers.push(spkr)
                    identified_speakers.push(spkr.id)
                  }
                })
                
                 setSpeakers(uniqueSpeakers.filter((s) => s.id == speakerId));
              }
            });
        },
        (apierror) => {
