      ...
      <div className={style.container}>
        ...
        <div className={style.footer}>
          {props.session !== null ? (
            <AppSessionToolbar
              ...
              menus={[
                { title: "Download", action: () => props.exportSession() },
                { title: "Graph", action: () => props.goToGraph() },
              ]}
            >
            </AppSessionToolbar>
          ) : (
        ...
        <div className={style["list-container"]}>
          ...
          Object.keys(props.sessionDevices).length === 0 ? (
            <div className={style["load-text"]}> No pods specified</div>
          ) : (
            <></>
          )}
          {props.sessionDevices !== null ? (
                ...
                >
                  ...
                  <svg>
                    <circle
                      ...
                      fillOpacity="0"
                      strokeWidth="3"
                      stroke={device.connected ? POD_ON_COLOR : POD_OFF_COLOR}
                    ></circle>
                  </svg>
                </svg>
                <div>{device.name}</div>
              ...
              <React.Fragment>
                <select id="ddDevice" className={style["dropdown-input"]}>
                  {props.devices.map((device, index) => (
                    <option key={index} defaultValue={device.id}>
                      {device.name}
                    </option>
                  ))}
                </select>
