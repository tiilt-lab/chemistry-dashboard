...
class ProcessingConfig:
    ...
    def from_json(data):
        ...
        if not auth_key or not sample_rate or not encoding or not channels:
            return False, "Start messege requires key, sample_rate, encoding, and channels."
...
# class ProcessingVideoConfig:
#     def __init__(self, auth_key, session_key, server_start, start_offset, audio_rate,video_rate, encoding, channels, transcribe, keywords, doa, features, tag, diarization):
#         self.auth_key = auth_key
#         self.session_key = session_key
#         self.server_start = server_start
#         self.start_offset = start_offset
...
#         encoding = data.get('encoding', None)
#         try:
#             audio_rate = int(data.get('audio_rate', None))
#         except Exception as e:
#             return False, "audio_rate must be an integer."
#         try:
#             video_rate = int(data.get('video_rate', None))
#         except Exception as e:
#             return False, "video_rate must be an integer."    
#         try:
#             channels = int(data.get('channels', None))
#         except Exception as e:
#             return False, "channels must be an integer."
...
#         # Check if fields are missing.
#         if not auth_key or not audio_rate or not video_rate or not encoding or not channels:
#             return False, "Start messege requires key, audio_rate, video_rate, encoding, and channels."
#         # Check if format is supported.
#         if not encoding in ['video/mp4', 'video/webm']:
#             return False, "Unsupported encoding type."
#         # Check if sample rate is supported.
#         if not audio_rate in [128000]:
#             return False, "Unsupported audio bit rate."
#         if not video_rate in [2500000]:
#             return False, "Unsupported video bit rate."    
#         # Check if tagging is requested and is possible.
#         tag = data.get('tag', False) and cf.record_original() and channels == 2
#         # Check if diarization is requested and possible.
#         diarization = data.get('tag', False) and channels == 6
...
#         return True, ProcessingVideoConfig(auth_key, session_key, server_start, start_offset, audio_rate, video_rate, encoding, channels, transcribe, keywords, doa, features, tag, diarization)
